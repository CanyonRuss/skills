...
Write it as a Markdown file titled `<topic>-handoff-<YYYYMMDD>.md`. Anchor it with the date and, if the user gave one, the stated focus of the next session.
...
- **Code state** — which files, modules, or areas of the codebase this work is scoped to, and anything mid-flight a fresh session needs to know before touching the code.
- **Test status** — what was passing, failing, or deliberately broken going into the next session. Don't leave a fresh session guessing whether a failure is pre-existing.
- **Environment / setup** — non-obvious requirements to run or test the code: env vars, required services, version pins. Anything that isn't in the code and would waste time to re-derive.
