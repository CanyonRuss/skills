...
description: >
  ...
  evaluating something that was built before the next phase rests on it — including reading a
  suite's output as evidence, and evaluating the suite itself as the thing that was built. Do NOT
  use for writing tests or test suites, or for a check small enough to simply run; the envelope is
  substantial evaluation work, not incidental verification. Apply when explicitly requested.
---
...
**Fix ground truth at entry.** State the baseline — the commit, the counts and the command that produces them, the shape of the surface as you first found it, whatever the claim will later be measured against — before it can drift. What is already broken or uncommitted when you arrive is part of that baseline, or it becomes yours by default.
...
**A subagent is a tool call here, so it is the rung to over-use rather than the one to reach for last.** It is a real reader outside your context and worth spending; it is still the second choice, and a judgment call about materiality or consequence goes to the person you are working with.
...
**Establish what an instrument reads before trusting a reading** — including the ones whose reading you already believe you know. A green suite, a coverage percentage, a clean lint, a passing CI run: for each, what it executed and what your claim covers are two different sets, and the gap is where the reading stops being evidence.

**When a result surprises you, settle apparatus or subject before anything else** — whether the defect is in the thing under test or in the thing doing the testing. A faulty instrument invalidates every result around it; a faulty subject is your finding. And when what broke is something the person you are working with controls — a drive that came unplugged, a credential that expired — say so and ask, rather than engineering around it. If it turns into a fault whose cause is unknown, that is diagnosis and it is their call whether to spend a session on it; hand it back rather than routing yourself into one.

## The suite is both instrument and subject

**Run it and read its output as evidence — and separately, evaluate it as the thing that was built.** Were the specified tests written? Where are the gaps? Writing them was the implementation session's job, and reviewing that work is squarely this one's.

**A test that exists and a test that discriminates are different things, and from an inventory they are indistinguishable.** Answering *did the tests get written* by enumeration is the substitution — the check is the discriminator question pointed at each test: *would this pass if the thing it claims to check were broken?* Routes that only ever ran against an empty library fall through to the same handler, so the path of interest was never exercised, and the suite reports green either way.
...
**Findings get recommendations, not repairs.** You recommend, you do not scope, and something that has an owner goes back to them with a price tag rather than getting quietly fixed on the way past. The version of this that will actually tempt you: finding a coverage gap, writing the missing test, watching it go green, and feeling finished. That is the fix substituting for the finding, and a missing test has an owner.

**A blocking defect goes back before the work is finished.** A check that failed is a finding — record it and keep going. One that stops you establishing anything further is not yours to root-cause: say what it blocks and hand it back with what you observed, what it took to observe it, and what it would cost someone to reproduce.
